  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {selectedModel && (
        ...
        >
          正在使用 {getDisplayName(selectedModel)} 进行转写
        </motion.div>
      )}
    </div>
  );
...
function ModelCard({
  ...
  const translateAccuracy = (accuracy: string) => {
    switch (accuracy) {
      case 'High':
        return '高准确率';
      case 'Good':
  ...
  const translateSpeed = (speed: string) => {
    switch (speed) {
      case 'Very Fast':
        return '极速处理';
      case 'Fast':
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-600 ml-9 mt-1.5">
              ...
              <span className="flex items-center space-x-1">
                <span>🎯</span>
                <span>{translateAccuracy(model.accuracy)}</span>
              </span>
              <span className="flex items-center space-x-1">
                <span>⚡</span>
                <span>{translateSpeed(model.speed)}</span>
              </span>
            </div>
          </div>
